...
class GAT(nn.Module):
    ...
    def forward(self, graph, inputs):
        h = self.node_encoder(inputs) 
        for layer in self.layers: 
            h_from_gat_layer = layer(
                graph,
                h
            )
            
            h = F.relu(h_from_gat_layer.mean(dim=1)) 
        return h 
...
class EdgeGAT(nn.Module):
    ...
    def forward(self, graph, node_inputs, edge_inputs):
        ...
        for layer in self.layers: 
            h_from_edge_gat_layer = layer(
                graph, 
                h,     
                e      
            )
            h = F.relu(h_from_edge_gat_layer.mean(dim=1))
...
class GINE(nn.Module):
    def __init__(
        ...
        self.node_encoder = nn.Linear(node_in_dim, node_hidden_dim)
        self.edge_encoder = nn.Linear(edge_in_dim, node_hidden_dim) 

        self.layers = nn.ModuleList([])
        self.layers.extend(
            [
                dglnn.GINEConv(
                    apply_func=nn.Linear(node_hidden_dim, node_hidden_dim), 
                    eps=0,           
                    learn_eps=False  
                )
                for _ in range(num_encoder_layers)
    ...
    def __init__(
        ...
        self.layers.extend(
            [
                ...
                dglnn.GatedGCNConv(
                    input_feats=node_hidden_dim,  
                    edge_feats=edge_hidden_dim,   
                    output_feats=node_hidden_dim, 
                    dropout=0.1,                  
                    batch_norm=True,              
                    activation=F.relu            
...
class ClassicGNN(nn.Module):
    ...
    def forward(self, g, num_nodes_per_graph):
        ...
        h = self.pred(h, labels, num_nodes_per_graph) 
        return h